    ...
    def do_GET(self):
        if self.path == "/mediathek/today":
            ...
            for i in range(7):
                ...
                url = f"https://www.freies-radio.de/mediathek?date={date_obj.strftime('%Y-%m-%d')}"
                ...
                for block in s.find_all("div", class_="views-row"):
                    ...
                    mu = f"https://www.freies-radio.de/systemfiles/mediathek/{dk}-{hk}.mp3"
                    b.append({"date":date_obj.strftime("%Y-%m-%d"),"start_time":st,"end_time":t[1].get("datetime",""),"broadcast_name":bn,"episode_title":et,"teaser":te,"mp3_url":mu})
                result.append({"date":date_obj.strftime("%Y-%m-%d"),"broadcasts":b})
